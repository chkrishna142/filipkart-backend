...
function signuporlogout() {
    const log = document.getElementById("loginname").innerHTML;

    if (log == "Login") {
        location.href = "signup.html";
    }
    else {
        document.getElementById("loginname").addEventListener("click", clearlocal);
...
//         }).then((res) => res.json()).then((res) => { document.getElementById("loginname").innerText = res.name }).catch((err) => { console.log(err) })
...
async function addToCart(e, i) {
    ...
    }).then((res) => res.json()).then((res) => { alert("item added to cart"); window.location.reload() }).catch((err) => { console.log(err) })
...
async function innercart() {
    ...
    }).then((res) => res.json()).then((res) => { res.map((e) => { count++; }) }).then(() => display(cart)).catch((err) => { console.log(err) })

    document.querySelector("#Sup").textContent = count;
